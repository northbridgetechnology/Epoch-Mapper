...
# Next standalone server bundle + static assets. `public/` carries the
# standalone-player template (public/player-template.html, generated by
# `npm run build:player` during `npm run build`) that the editor's "Export
# Standalone HTML" fetches, so it must ship in the image. A tracked
# public/.gitkeep guarantees the directory exists in the build context.
